...
(function() {
  ...
  */
  ...
  function displayHistory() {
        ...
        config.chatHistory.forEach(msg => {
            ...
            // 为历史消息添加选项菜单
            addMessageOptionsMenu(msgDiv,msg.role);
        });
  ...
  */
  ...
  async function sendMessage(message, retryCount = 0, isSummaryTask = false) {
    ...
    // 为用户消息添加选项菜单
    addMessageOptionsMenu(userMsgDiv,"user");
    ...
    // 为AI消息添加选项菜单
    addMessageOptionsMenu(aiMsgDiv,"ai");
  ...
  */
  ...
  // 为消息添加三点菜单和复制功能
  function addMessageOptionsMenu(messageDiv,role) {
    // 创建容器使消息可以相对定位
    const messageContainer = document.createElement('div');
    messageContainer.className = `ds-${role}-message`;
